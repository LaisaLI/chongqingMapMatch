...
public class NextLineOper {
	...
	//	this.path2 = "D:/EclipseWS/MapMatch/Data/NextLineResult.txt";///////////
		this.path1 = "G://Workplace//MapMatch//Data//mapIndex.txt";//已改为从config文件中读取
		this.path2 = "G://Workplace//MapMatch//Data//NextLineResult.txt";//已改为从config文件中读取
	...
	public Scanner openFile(String path){
		...
		if(f1.exists() && !f1.isDirectory()){
			try {
				...
				System.out.println(path + "打开失败！" + "----" + e.toString());
			}
		}
		return scanner;
	}
...
/*	public void setPosition(String sPath,String dPath){
		...
		try {
			...
			// TODO Auto-generated catch block
			System.out.println(sPath + "打开失败！" + "----" + e.toString());
		}
	}*/
	...
	public long twoDivide(String _node) throws IOException
	{
		...
		int posInArl = map.get(_node.substring(0, 6));//posInArl记录submap的位置；
		if (preGridId != null && !preGridId.equals(_node.substring(0, 6))) record.clear();
		Long node = Long.parseLong(_node);
		long font = arl.get(posInArl);//font记录submap中node的个数；
		long tail =  0;
		//long offset = 0;
		...
		//找出k使得getNodeId(k*LINE)>=node>getNodeId((k-1)*LINE)
		while (font < tail)
		{
			mid = (font + tail) / 2;
